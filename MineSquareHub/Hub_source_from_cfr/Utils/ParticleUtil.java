...
public class ParticleUtil {
    ...
    public static enum ParticleEffect {
        ...
        private /* varargs */ ParticleEffect(String name, int id, String requiredVersion, int properties, int n2, ParticleProperty ... arrparticleProperty) {
            this.name = name;
            this.id = id;
        ...
        public static final class ParticlePacket {
            ...
            public static void initialize() throws VersionIncompatibleException {
                ...
                try {
                    version = Integer.parseInt(Character.toString(ReflecionUtils.PackageType.getServerVersion().charAt(3)));
                    if (version > 7) {
                        enumParticle = ReflecionUtils.PackageType.MINECRAFT_SERVER.getClass("EnumParticle");
                    }
                    Class packetClass = ReflecionUtils.PackageType.MINECRAFT_SERVER.getClass(version < 7 ? "Packet63WorldParticles" : "PacketPlayOutWorldParticles");
                    packetConstructor = ReflecionUtils.getConstructor(packetClass, new Class[0]);
                    getHandle = ReflecionUtils.getMethod("CraftPlayer", ReflecionUtils.PackageType.CRAFTBUKKIT_ENTITY, "getHandle", new Class[0]);
                    playerConnection = ReflecionUtils.getField("EntityPlayer", ReflecionUtils.PackageType.MINECRAFT_SERVER, false, "playerConnection");
                    sendPacket = ReflecionUtils.getMethod(playerConnection.getType(), "sendPacket", ReflecionUtils.PackageType.MINECRAFT_SERVER.getClass("Packet"));
                }
            ...
            private void initializePacket(Location center) throws PacketInstantiationException {
                ...
                try {
                    ...
                    if (version < 8) {
                        ...
                        ReflecionUtils.setValue(this.packet, true, "a", name);
                    } else {
                        ReflecionUtils.setValue(this.packet, true, "a", enumParticle.getEnumConstants()[this.effect.getId()]);
                        ReflecionUtils.setValue(this.packet, true, "j", this.longDistance);
                        if (this.data != null) {
                            int[] arrn;
                            ...
                            ReflecionUtils.setValue(this.packet, true, "k", arrn);
                        }
                    }
                    ReflecionUtils.setValue(this.packet, true, "b", Float.valueOf((float)center.getX()));
                    ReflecionUtils.setValue(this.packet, true, "c", Float.valueOf((float)center.getY()));
                    ReflecionUtils.setValue(this.packet, true, "d", Float.valueOf((float)center.getZ()));
                    ReflecionUtils.setValue(this.packet, true, "e", Float.valueOf(this.offsetX));
                    ReflecionUtils.setValue(this.packet, true, "f", Float.valueOf(this.offsetY));
                    ReflecionUtils.setValue(this.packet, true, "g", Float.valueOf(this.offsetZ));
                    ReflecionUtils.setValue(this.packet, true, "h", Float.valueOf(this.speed));
                    ReflecionUtils.setValue(this.packet, true, "i", this.amount);
                }
                catch (Exception exception) {
